...
Application::Application()
    : m_clock(core::ProjectSettings::get().physics.fixed_timestep)
{
}
...
int Application::run(int argc, char** argv) {
    ...
    if (!m_headless_mode) {
        if (!create_window(settings().window)) {
            core::log(core::LogLevel::Error, "Failed to create window");
            return 1;
        }
    } else {
        ...
        core::log(core::LogLevel::Info, "Running in headless mode ({}x{})", m_window_width, m_window_height);
    }
    ...
    if (!m_game_dll_path.empty()) {
        if (!load_game_plugin(m_game_dll_path)) {
            core::log(core::LogLevel::Error, "Failed to load game plugin: {}", m_game_dll_path.string());
    ...
    while (!m_quit_requested) {
        ...
        // Update time
        core::Time::update();
        double dt = core::Time::delta_time();
...
bool Application::load_game_plugin(const std::filesystem::path& dll_path) {
    ...
    if (!std::filesystem::exists(dll_path)) {
        core::log(core::LogLevel::Error, "Game plugin not found: {}", dll_path.string());
        return false;
    }
    ...
    core::log(core::LogLevel::Info, "Game plugin loaded: {}", dll_path.string());
    return true;
}
...
bool Application::create_window(const core::WindowSettings& ws) {
    m_platform_window = core::create_platform_window();
    if (!m_platform_window) {
        core::log(core::LogLevel::Error, "No windowing backend available");
        return false;
    }

    core::WindowCallbacks callbacks;
    callbacks.on_close = []() {
        core::events().dispatch(core::WindowCloseEvent{});
    };
    callbacks.on_resize = [this](uint32_t w, uint32_t h) {
        m_window_width = w;
        m_window_height = h;
        core::events().dispatch(core::WindowResizeEvent{w, h});
    };
    callbacks.on_focus = [](bool focused) {
        core::events().dispatch(core::WindowFocusEvent{focused});
    };
