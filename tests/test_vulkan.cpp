...
TEST(VulkanRenderPass_CreatesColorDepthPass) {
    ...
    depthBuffer.Init(&context, &swapchain);

    VulkanRenderPass renderPass;
    renderPass.Init(&context, &swapchain, depthBuffer.GetFormat());

    ASSERT(renderPass.Get() != VK_NULL_HANDLE);
    ASSERT(renderPass.GetDepthFormat() == depthBuffer.GetFormat());
    ASSERT(depthBuffer.GetImageView() != VK_NULL_HANDLE);

    VulkanFramebuffer framebuffer;
    framebuffer.Init(&context, &swapchain, &renderPass, depthBuffer.GetImageView());
